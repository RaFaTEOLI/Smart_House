<?php
  ...
  if (isset($_GET["success"])) {
    if ($_GET["success"] == 1) {
      echo '<input type="hidden" id="pessoaInserida" value="true">';
    } else {
      echo '<input type="hidden" id="pessoaInserida" value="false">';
    }
  }
          ...
          <div class="card">
            <div class="card-header">
              <div class="d-flex justify-content-between">
                ...
                <a type="button" href="cadastrar_usuario.php" class="btn btn-secondary btn-sm"><i class="fas fa-plus"></i></a>
              </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="ativacoes" class="table table-bordered table-striped table-responsive-sm">
                ...
                <tbody>
                <?php while ($linha = mysqli_fetch_assoc($pessoas)) { ?>
                  <tr>
                    <td align="center"><img height="40" width="40" src="<?php echo $linha["foto"] ? $linha["foto"] : "/smart_house/dist/img/avatar5.png" ?>" class="img-circle elevation-2" alt="User Image"></td>
                    <td><?= $linha["nome"] ?></td>
                    <td><?= $linha["sobrenome"] ?></td>
                    <td class="d-none d-lg-block"><?= $linha["email"] ?></td>
                    <td align="center">
                      <div class="btn-group">
                          <a type="button" href="alterar_usuario.php?id=<?= $linha["pessoaId"] ?>" class="btn btn-warning"><i class="fas fa-edit"></i></a>
                          <a type="button" href="excluir_usuario.php?id=<?= $linha["pessoaId"] ?>" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                      </div>
                    </td>
                  </tr>
                <?php } ?>
...
<script>
  ...
  $(function() {
    ...
    const pessoaInserida = $('#pessoaInserida').val();

    if (pessoaInserida == "true") {
      printToast("Ação realizada!", "success");
    } else if (pessoaInserida == "false") {
