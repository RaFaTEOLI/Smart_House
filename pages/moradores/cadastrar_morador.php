<?php
  ...
  if (isset($_POST["pessoa"]) && isset($_POST["casaId"])) {
    if(!$daoMorador->validarMorador($conn, $_POST["casaId"], $_POST["pessoa"])) {

      echo '<input type="hidden" id="moradorDuplicado" value="false">';

      if($daoMorador->salvarMorador($conn, $_POST)) {
        header("Location: moradores.php?success=1");
      }
    } else {
      echo '<input type="hidden" id="moradorDuplicado" value="true">';
    }
          ...
          <div class="card">
            <div class="card-header">
              <div class="d-flex justify-content-between">
                <h3 class="card-title">Cadastrar Morador</h3>
                <a type="button" href="moradores.php" class="btn btn-secondary btn-sm"><i class="fas fa-undo"></i></a>
              </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <form role="form" id="quickForm" action="cadastrar_morador.php" method="POST">
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-12">
                    ...
                    <input type="hidden" value="<?= $_SESSION["casaId"] ?>" name="casaId">
                    <div class="col-md-6">
...
$(document).ready(function () {
  ...
  $('#quickForm').validate({
    ...
    messages: {
      pessoa: {
        required: "Por favor selecione uma pessoa!",
      },
    },
    errorElement: 'span',
...
$(function() {
    ...
    const moradorDuplicado = $('#moradorDuplicado').val();

    if (moradorDuplicado == "true") {
      printToast("Morador Duplicado!", "error");
    }
  });
</script>
<!-- JQVMap -->
