...
$mensagensQuery = $daoMensagem->getMensagens($conn, $_SESSION["casaId"]);

if (isset($_GET["success"])) {
  if ($_GET["success"] == 1) {
    echo '<input type="hidden" id="mensagemLida" value="true">';
  } else {
    echo '<input type="hidden" id="mensagemLida" value="false">';
  }
}
?>
<!DOCTYPE html>
...
<body class="hold-transition sidebar-mini layout-fixed">
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                <?php
                if (mysqli_num_rows($mensagensQuery) > 0) {
                  while ($mensagem = mysqli_fetch_assoc($mensagensQuery)) {
                ?>
                    <div class="card bg-warning" style="width: 100%; border-radius: 39px 39px 39px 39px;">
                      <div class="card-body">
                        ...
                        <a type="button" class="btn btn-outline-dark" href="/smart_house/pages/mensagens/vizualizar_mensagem.php?id=<?php echo $mensagem["mensagemId"] ?>" class="card-link"><i class="fas fa-eye"></i> Marcar como lida</a>
                      </div>
                    </div>
                    <br>
                  <?php
                  ...
                  ?>
                  Não há mensagens para vizualizar
                <?php
                }
                ?>
              </div>
  ...
  <script>
    ...
    $(function() {
      ...
      const mensagemLida = $('#mensagemLida').val();

      if (mensagemLida == "true") {
        printToast("Ação realizada!", "success");
      } else if (mensagemLida == "false") {
